...
public class Page {
    ...
    * If the charset is US-ASCII, then each char
   ...
   public static final int STR_SIZE(int n) {
      float bytesPerChar = SDBcharset.newEncoder().maxBytesPerChar();
   ...
   public synchronized String getString(int offset) {
      ...
      contents.get(byteval);
      	return new String(byteval, SDBcharset);
   }
   ...
   public synchronized void setString(int offset, String val) {
      contents.position(offset);
      byte[] byteval = val.getBytes(SDBcharset);
